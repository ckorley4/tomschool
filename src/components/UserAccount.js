...
const UserAccount = ({ isLoggedIn, onLogout, addCourse }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (response.ok) {
      const addCourse = await response.json()
      ...
      // if (addCarToList) {
      // add(addedCar.car) // Our backend returns the added car under the 'car' key
      //}
  ...
  return (
    <div className="user-account-container">
      ...
      <form onSubmit={handleSubmit}>
        {submitSuccess && (
          <div className="success-message">Enrollment successfully added!</div>
        )}{' '}
